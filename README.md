...
默认直接随机出题，按 `n` 生成下一道随机题，按 `p` 回到本次运行的上一题。已有未完成进度时继续当前题；完成后再次启动会出新题。`--random` 强制开始新题，`--exercise` 按名称从当前题集选题，`--list` 列出当前题集里该语言的题目：
