...
开发环境需要 Rust。已编译的程序可直接运行；首次 Cargo 构建需要下载依赖。运行时必须选择 `--python` 或 `--logic`，不能同时指定；自然演绎使用 `--logic --proof …`。`--help` 和 `--version` 无需选择语言。题目来自当前题集：默认是内置题集，`--set FILE` 换成外部 TOML 题集，格式见下文「题集文件」。
...
默认直接随机出题，按 `n` 生成下一道随机题，按 `p` 回到本次运行的上一题。已有未完成进度时继续当前题；完成后再次启动会出新题。`--random` 强制开始新题，`--exercise` 按 ID 从当前题集选题，`--list` 列出当前题集里该语言的题目：
...
题集是一份带版本号的 TOML 文件：一个稳定的题集 ID，加上若干道题的题面、语言、表达式或公式和变量取值。内置题集本身就是这个格式（`assets/exercises.toml`），和导入的文件共用同一条加载路径、同一套检查，没有第二个格式，也没有兼容读法。`--set FILE` 用文件里的题集取代内置题集，此后 `--list`、`--trace`、`--exercise`、`--assign`、`--evaluation` 都作用于这份题集；`--python` / `--logic` 在题集内部筛选语言。`--set` 不能与自定义表达式、`--random`、`--seed`、`--proof`、`--goal`、`--premise`、`--check-proof`、`--equivalent` 同用：这些各自指定另一个题目来源，程序直接拒绝，不会收下再丢掉。用 `--exercise` 打开证明题时，`--assign` 和 `--evaluation` 对它没有意义，同样拒绝。

`examples/questions.toml` 是一份可直接运行的完整示例，字段如下：

```toml
version = 1                    # 只认 1；写别的版本会被拒绝，而不是猜着读
id = "example-set"             # 题集的稳定身份，进度按它保存
title = "示例题集"
description = "可选的一句话说明"

[[questions]]
kind = "evaluation"            # 必填并写明，不按出现了哪些字段推断
id = "literal-types"           # 题集内唯一，非空
title = "赋值写的是源码字面量"
...
premises = ["P -> Q", "Q -> R", "P"]
conclusion = "R"               # 要推出的公式
goal = "每行写「公式 ; 规则 ; 引用行」；规则、引用行和假设作用域都会被检查。"
```

`kind` 决定这道题有哪些字段，另一类的字段会被拒绝而不是忽略：求值题写 `premises`、证明题写 `evaluation` 都报未知字段。短路是求值策略，对证明没有意义。`premises` 和 `conclusion` 是公式，`goal` 两类题都一样，是题面文字。
...
身份与进度：认的是题集 `id`，不是文件路径。文件改名、换目录、复制到别处都不影响已有记录，改 `id` 才算另一套题；反过来，两份文件写同一个 `id` 就是同一套题，`id` 是一个全局名字。`builtin` 留给内置题集，导入的文件不能用。进度指针记的是「哪个题集的哪道题」，所以两套题集里都叫 `q1` 的题目不会互相打开对方的进度。每道题的答题记录按内容保存（教学规则版本、策略、语言、赋值、表达式），改标题或题面文字不影响记录，改表达式或赋值则是另一道题：它从头开始，旧记录仍留在进度文件里。题集加载失败时程序在解析进度文件路径之前就退出，不会碰已有进度。

加载时逐项检查，任何一项不通过就拒绝整份文件，并指出文件、出错的题目和字段（题集自身的字段不带题目）：TOML 语法错误、未知字段和写错类型的字段由 toml 自己报出，带行号和列号；版本号不是 1、题集 ID 或标题为空、题集没有题目、题目 ID 为空或重复、题目标题为空、取值不是该语言的字面量、表达式或公式解析不了，各报一句话。题集最多 1024 道题、1 MiB；证明题最多 64 条前提；表达式、公式和证明自身的规模上限见下文。以异常结束的题目（例如 `6 / 0`）是合法题目，照常加载。

带 `--set` 时，练习按文件顺序走这份题集中当前语言的求值题：`n` 下一题，`p` 上一题，最后一题再按 `n` 只提示「已经是本题集的最后一题。」，不会转而生成随机题。证明题不在这条顺序里，用 `--exercise ID` 单独打开。启动时从进度指向的那道题往后找第一道还没做完的题；整套都做完了就停在最后一题。进度只记一个指针，所以在两套题集之间来回切换会丢掉离开那套时的位置——再回去时是从头查找未完成的题，而不是把已完成的第一题当新题递回来。不带 `--set` 的行为不变：默认随机出题，`n` 仍生成新的随机题。

```fish
cargo run --locked -- --python --set examples/questions.toml --list
cargo run --locked -- --logic --set examples/questions.toml --list
cargo run --locked -- --python --set examples/questions.toml
cargo run --locked -- --logic --set examples/questions.toml
cargo run --locked -- --logic --set examples/questions.toml --exercise chain
cargo run --locked -- --python --set examples/questions.toml --exercise eager-division --trace
```
...
默认在系统本地应用数据目录的 Stepwise 目录保存 `progress.json`。`--progress-file PATH` 指定位置，`--no-save` 禁用读取和保存。正确步骤、撤销和换题后原子写入；重启通过重放步骤恢复历史。不同表达式、赋值（包括类型、浮点负零）、模式、教学规则版本分别保存；自然演绎按前提与目标隔离。进度指针同时记下题集 ID 与题目 ID，题集按 ID 认身份而不按文件路径。统一代入、同优先级任选及最终负号完成的新规则使用独立进度，旧记录保留，不重放旧版多出来的填写；自然演绎进度不变。损坏或无法重放的记录明确报错，不会静默覆盖。进度文件自带版本号：题集之前的第 1 版指针只写了题目、没写题集，本版读不了，会报错并原样留下该文件，请改用 `--progress-file` 指向新位置或 `--no-save`。首版同一个进度文件只供一个程序实例使用。
...
架构：`src/core` 提供与语言无关的教学机制：稳定节点 ID、树替换与来源映射、可选步骤检查、按类型的反馈、历史与重放；`src/python` 提供 Python 解析、运算规则、类型语义、优先级、短路规则、解释与出题语法；`src/logic` 提供符号解析、命题语义、BDD 检查、自然演绎与出题语法；`src/generate.rs` 提供共用的种子协议、采样流程与可完成性检查；`src/app` 是与界面无关的应用层，负责题目来源与换题、当前会话的选择与草稿、撤销、重新开始、策略切换、历史归档与进度快照，不依赖任何终端或窗口库；`src/tui` 只做适配：把终端事件映射为应用层操作并绘制其状态；`src/exercises.rs` 定义题集格式并解析 TOML，内置题集 `assets/exercises.toml` 与 `--set` 导入的文件走同一条加载路径；`src/progress.rs` 保存并重放进度。core 只通过 `Language` 与 `Op` 两个小接口调用语言模块，不内联任何一种语言的规则。Ratatui + Crossterm 提供终端界面，rustpython-parser 只负责 Python 解析。
