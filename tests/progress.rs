...
use stepwise::{
	core::{EvaluationMode, RecordedAttempt, Session, Value, parse_value},
	progress::Progress,
};
...
fn atomic_round_trip_and_mode_assignment_isolation() {
	...
	let mut short: Session =
		Session::new("False and (1 / 0)", EvaluationMode::ShortCircuit).unwrap();
	...
	let eager: Session = Session::new(short.source(), EvaluationMode::Eager).unwrap();
...
fn final_negative_rule_preserves_older_progress_without_replaying_the_extra_answer() {
	let mut session: Session = Session::new("-2 ** 2", EvaluationMode::ShortCircuit).unwrap();
	...
	let restored: Session = Session::new(session.source(), session.mode())
		.unwrap()
...
fn group_clicks_round_trip_without_fabricated_input_and_cannot_solve_operations() {
	let directory: tempfile::TempDir = tempfile::tempdir().unwrap();
	let path: std::path::PathBuf = directory.path().join("progress.json");
	let mut session: Session = Session::new("((3))", EvaluationMode::ShortCircuit).unwrap();
	...
	let mut arithmetic: Session = Session::new("2 + 3", EvaluationMode::ShortCircuit).unwrap();
...
fn python_assignments_types_and_signed_zero_have_separate_progress() {
	...
	for literal in ["False", "0", "0.0", "-0.0", "None", "2", "2.0"] {
		...
		let mut session: Session =
			Session::with_bindings("(x)", &bindings, EvaluationMode::ShortCircuit).unwrap();
		assert!(progress.attempts(&session).is_empty());
		let id: usize = session.next_step().unwrap().node_id;
		assert!(session.submit(id, literal).accepted());
		progress.record("typed", &session);
		let restored: Session =
			Session::with_bindings("(x)", &bindings, EvaluationMode::ShortCircuit)
				.unwrap()
				.replay(progress.attempts(&session))
				.unwrap();
